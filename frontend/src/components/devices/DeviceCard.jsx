...
export default function DeviceCard({ name, os, lastSync, status, type }) {
  const Icon = DEVICE_ICONS[type] || MdComputer;
  const colorClass = DEVICE_COLORS[type] || DEVICE_COLORS.windows;
  ...
  return (
    <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow group">
      <div className="flex items-start justify-between mb-6">
        ...
        <button className="text-slate-400 hover:text-primary transition-colors p-1" title="Remove Device">
          <MdDeleteOutline />
        </button>
      </div>
      <div className="mb-4">
